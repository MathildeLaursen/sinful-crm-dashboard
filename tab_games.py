"""
Games Tab - CRM Dashboard
Med sub-tabs for hvert game
"""
...
def load_games_data():
    """Henter Games data fra Google Sheet"""
    try:
        ...
        worksheet = spreadsheet.worksheet("All_Game")
    ...
    for country_code, start_col in country_configs:
        try:
            ...
            country_df['Game'] = raw_df.iloc[:, 2]           # C: Game
    ...
    # Opret Game-Trigger identifier
    df['Game_Trigger'] = df['Game'].astype(str).str.strip() + ' - ' + df['Trigger'].astype(str).str.strip()
...
def aggregate_to_game_level(df):
    """Aggreger data til game niveau (summer alle mails under samme game)"""
    agg_df = df.groupby(['Year_Month', 'Game_Trigger', 'Country'], as_index=False).agg({
        'Received_Email': 'sum',
        'Total_Opens': 'sum',
        'Unique_Opens': 'sum',
        'Total_Clicks': 'sum',
...
def get_unique_games(df):
    """Hent unikke games sorteret efter game nummer (ekskluderer ikke-aktive games)"""
    # Flows der ikke længere er aktive
    INACTIVE_FLOWS = {0, 1, 2, 3, 9}
    
    def game_sort_key(f):
        match = re.search(r'Game\s*(\d+)', f)
        return int(match.group(1)) if match else 999
    
    def is_active_game(f):
        match = re.search(r'Game\s*(\d+)', f)
        if match:
    ...
    flows = df['Game_Trigger'].unique()
    active_games = [f for f in flows if is_active_game(f)]
    return sorted(active_games, key=game_sort_key)
...
def render_overview_content(game_df, sel_countries, sel_games, full_df=None, all_months_df=None, ignore_inactive=True, active_games=None):
    ...
    current_df = game_df[
        ...
        (game_df['Game_Trigger'].isin(sel_games))
    ].copy()
    ...
    display_df = current_df.groupby(['Year_Month', 'Game_Trigger'], as_index=False).agg({
        'Received_Email': 'sum',
        'Total_Opens': 'sum',
        'Unique_Opens': 'sum',
        'Total_Clicks': 'sum',
    ...
    if full_df is not None:
        ...
        prev_df = full_df[
            ...
            (full_df['Game_Trigger'].isin(sel_games))
        ].copy()
    ...
    def chart_game_sort_key(f):
        match = re.search(r'Game\s*(\d+)', f)
        return int(match.group(1)) if match else 999
    unique_games = sorted(chart_df['Game_Trigger'].unique(), key=chart_game_sort_key)

    fig = go.Figure()
    
    # Tilføj linje for hvert flow
    for idx, game in enumerate(unique_games):
        game_data = chart_df[chart_df['Game_Trigger'] == game].sort_values('Sort_Key')
        short_name = get_short_game_name(game)
        ...
        fig.add_trace(
            go.Scatter(
                x=game_data['Month_Label'], 
                y=game_data['Received_Email'],
                name=short_name,
                mode='lines+markers',
    ...
    table_df = display_df.groupby('Game_Trigger', as_index=False).agg({
        'Received_Email': 'sum',
        'Unique_Opens': 'sum',
        'Unique_Clicks': 'sum',
        'Unsubscribed': 'sum',
    ...
    table_df['_game_num'] = table_df['Game_Trigger'].apply(lambda f: int(re.search(r'Game\s*(\d+)', f).group(1)) if re.search(r'Game\s*(\d+)', f) else 999)
    table_df = table_df.sort_values('_game_num', ascending=True)
    table_df = table_df.drop(columns=['_game_num'])
    
    # Vælg og sorter kolonner
    table_df = table_df[['Game_Trigger', 'Received_Email', 'Unique_Opens', 'Unique_Clicks', 'Open_Rate', 'Click_Rate', 'CTR', 'Unsubscribed', 'Bounced']]
    ...
    st.dataframe(
        ...
        column_config={
            "Game_Trigger": st.column_config.TextColumn("Flow", width="large"),
...
def render_single_game_content(raw_df, game_trigger, sel_countries, filter_config=None, full_df=None):
    """Render indhold for et enkelt game med mail-niveau breakdown"""
    # Filtrer til dette specifikke flow og valgte lande
    game_data = raw_df[
        (raw_df['Game_Trigger'] == game_trigger) &
        (raw_df['Country'].isin(sel_countries))
    ].copy()
    
    if game_data.empty:
        st.warning(f"Ingen data for {game_trigger} med de valgte filtre.")
        return
    ...
    if filter_config is not None:
        if filter_config.get('mails'):
            ...
            if include_blank:
                game_data = game_data[game_data['Mail'].isin(non_blank_mails) | (game_data['Mail'].str.strip() == '') | game_data['Mail'].isna()]
            else:
                game_data = game_data[game_data['Mail'].isin(non_blank_mails)]
        if filter_config.get('groups'):
            ...
            if include_blank:
                game_data = game_data[game_data['Group'].isin(non_blank_groups) | (game_data['Group'].str.strip() == '') | game_data['Group'].isna()]
            else:
                game_data = game_data[game_data['Group'].isin(non_blank_groups)]
        if filter_config.get('messages'):
            ...
            if include_blank:
                game_data = game_data[game_data['Message'].isin(non_blank_messages) | (game_data['Message'].str.strip() == '') | game_data['Message'].isna()]
            else:
                game_data = game_data[game_data['Message'].isin(non_blank_messages)]
        if filter_config.get('ab'):
            ...
            if include_blank:
                game_data = game_data[game_data['AB'].isin(non_blank_ab) | (game_data['AB'].str.strip() == '') | game_data['AB'].isna()]
            else:
                game_data = game_data[game_data['AB'].isin(non_blank_ab)]
    ...
    scorecard_df = game_data.groupby(['Year_Month'], as_index=False).agg({
        'Received_Email': 'sum',
        'Unique_Opens': 'sum',
        'Unique_Clicks': 'sum',
        'Unsubscribed': 'sum',
    ...
    if full_df is not None:
        ...
        prev_filter = (
            (full_df['Year_Month'].isin(prev_months)) &
            (full_df['Game_Trigger'] == game_trigger) &
            (full_df['Country'].isin(sel_countries))
        )
    ...
    mail_df = game_data.groupby(group_cols, as_index=False).agg({
        'Received_Email': 'sum',
        'Total_Opens': 'sum',
        'Unique_Opens': 'sum',
        'Total_Clicks': 'sum',
    ...
    if filter_config is not None and '_ignore_inactive_key' in filter_config:
        ...
        ignore_inactive_new = st.checkbox(
            ...
            key=f"gm_ignore_inactive_cb_{game_trigger}"
        )
    ...
    if not table_group_cols:
        ...
        game_data['_dummy'] = 'Total'
        table_group_cols = ['_dummy']
    
    # Brug game_data direkte (allerede filtreret) - aggreger kun på tværs af måneder
    table_df = game_data.groupby(table_group_cols, as_index=False).agg({
        'Received_Email': 'sum',
        'Unique_Opens': 'sum',
        'Unique_Clicks': 'sum',
        'Unsubscribed': 'sum',
    ...
    all_months_for_chart = full_df[full_df['Game_Trigger'] == game_trigger]['Year_Month'].unique()
...
def get_short_game_name(game_trigger):
    """Udtræk kun 'Flow X' fra 'Flow X - Trigger Name'"""
    match = re.search(r'(Game\s*\d+)', game_trigger)
    return match.group(1) if match else game_trigger


def render_games_tab():
    """Render Games tab indhold med sub-tabs for hvert game"""
    ...
    try:
        with st.spinner('Henter game data...'):
            df = load_games_data()
        if df.empty:
            st.error("Kunne ikke hente game data. Tjek Google Sheets konfiguration.")
            return
    except Exception as e:
        st.error(f"Fejl: {e}")
        return
    ...
    if 'gm_selected_countries' not in st.session_state:
        st.session_state.gm_selected_countries = None
    if 'gm_selected_games' not in st.session_state:
        st.session_state.gm_selected_games = None
    if 'gm_cb_reset_land' not in st.session_state:
        st.session_state.gm_cb_reset_land = 0
    if 'gm_cb_reset_game' not in st.session_state:
        st.session_state.gm_cb_reset_game = 0

    # Hent alle flows først (før filtrering) for at kunne vise subtabs
    all_game_triggers = get_unique_games(df)
    
    # Sub-tabs: Oversigt + et tab per flow (med korte navne)
    tab_labels = ["Oversigt"] + [get_short_game_name(f) for f in all_game_triggers]
    ...
    for i, game_trigger in enumerate(all_game_triggers):
        with sub_tabs[i + 1]:
            render_single_game_tab_content(df, game_trigger, available_months)
...
def render_overview_tab_content(df, available_months):
    ...
    full_game_df = aggregate_to_game_level(df)
    all_countries = sorted(full_game_df['Country'].unique())
    all_games_full = get_unique_games(full_game_df)
    ...
    recent_months = [current_month, prev_month]
    recent_data = full_game_df[
        (full_game_df['Year_Month'].isin(recent_months)) & 
        (full_game_df['Received_Email'] > 0)
    ]
    ...
    if st.session_state.gm_selected_countries is None:
        st.session_state.gm_selected_countries = list(all_countries)
    else:
        st.session_state.gm_selected_countries = [c for c in st.session_state.gm_selected_countries if c in all_countries]

    if st.session_state.gm_selected_games is None:
        st.session_state.gm_selected_games = list(all_games)
    else:
        st.session_state.gm_selected_games = [f for f in st.session_state.gm_selected_games if f in all_games]
        if not st.session_state.gm_selected_games:
            st.session_state.gm_selected_games = list(all_games)

    # Layout - dropdowns, checkbox og slider på samme linje
    col_land, col_game, col_inaktive, col_slider = st.columns([1, 1, 0.8, 3.2])

    # Land filter
    with col_land:
        land_count = len(st.session_state.gm_selected_countries)
        ...
        with st.popover(land_label, use_container_width=True):
            reset_land = st.session_state.gm_cb_reset_land
            all_land_selected = len(st.session_state.gm_selected_countries) == len(all_countries)
            select_all_land = st.checkbox("Vælg alle", value=all_land_selected, key=f"gm_sel_all_land_{reset_land}")
            ...
            for country in all_countries:
                ...
                with cb_col:
                    checked = country in st.session_state.gm_selected_countries
                    if st.checkbox(country, value=checked, key=f"gm_cb_land_{country}_{reset_land}"):
                        new_selected.append(country)
                with only_col:
                    if st.button("Kun", key=f"gm_only_land_ov_{country}_{reset_land}", type="secondary"):
                        only_clicked_land = country
            
            # Handle Kun button click first
            if only_clicked_land:
                st.session_state.gm_selected_countries = [only_clicked_land]
                st.session_state.gm_cb_reset_land += 1
                st.rerun()
                ...
                st.session_state.gm_selected_countries = []
                st.session_state.gm_cb_reset_land += 1
                st.rerun()
            elif set(new_selected) != set(st.session_state.gm_selected_countries):
                st.session_state.gm_selected_countries = new_selected
                st.session_state.gm_cb_reset_land += 1
                st.rerun()

    # Flow filter
    with col_game:
        game_count = len(st.session_state.gm_selected_games)
        game_label = f"Game ({game_count})" if game_count < len(all_games) else "Game"
        with st.popover(game_label, use_container_width=True):
            reset_game = st.session_state.gm_cb_reset_game
            all_game_selected = len(st.session_state.gm_selected_games) == len(all_games)
            select_all_game = st.checkbox("Vælg alle", value=all_game_selected, key=f"gm_sel_all_game_{reset_game}")
            
            new_selected_games = []
            only_clicked_game = None
            for game in all_games:
                cb_col, only_col = st.columns([4, 1])
                with cb_col:
                    # Vis kun kort flow-navn i checkboxen
                    short_name = get_short_game_name(game)
                    checked = game in st.session_state.gm_selected_games
                    if st.checkbox(short_name, value=checked, key=f"gm_cb_game_{game}_{reset_game}"):
                        new_selected_games.append(game)
                with only_col:
                    if st.button("Kun", key=f"gm_only_game_{game}_{reset_game}", type="secondary"):
                        only_clicked_game = game
            
            # Handle Kun button click first
            if only_clicked_game:
                st.session_state.gm_selected_games = [only_clicked_game]
                st.session_state.gm_cb_reset_game += 1
                st.rerun()
            elif select_all_game and not all_game_selected:
                st.session_state.gm_selected_games = list(all_games)
                st.session_state.gm_cb_reset_game += 1
                st.rerun()
            elif not select_all_game and all_game_selected:
                st.session_state.gm_selected_games = []
                st.session_state.gm_cb_reset_game += 1
                st.rerun()
            elif set(new_selected_games) != set(st.session_state.gm_selected_games):
                st.session_state.gm_selected_games = new_selected_games
                st.session_state.gm_cb_reset_game += 1
                st.rerun()
    ...
    with col_inaktive:
        ignore_inactive = st.checkbox(
            "Ignorer Inaktive", 
            value=st.session_state.gm_ignore_inactive_overview, 
            key="gm_ignore_inactive_overview_cb"
        )
        if ignore_inactive != st.session_state.gm_ignore_inactive_overview:
            st.session_state.gm_ignore_inactive_overview = ignore_inactive
            # Reset flow selection når toggle ændres
            new_all_games = [f for f in all_games_full if f in active_games] if ignore_inactive else all_games_full
            st.session_state.gm_selected_games = list(new_all_games)
            st.session_state.gm_cb_reset_game += 1
            st.rerun()
    ...
    with col_slider:
        if len(sorted_months) > 1:
            ...
            saved_key = "gm_month_range_overview_saved"
    ...
    game_df = aggregate_to_game_level(df_month_filtered)

    # Check selections
    sel_countries = st.session_state.gm_selected_countries
    sel_games = st.session_state.gm_selected_games
    ...
    if not sel_games:
        st.warning("Vælg mindst ét game.")
        return
...
def render_single_game_tab_content(df, game_trigger, available_months):
    """Render enkelt game tab med filtre og indhold"""
    ...
    game_data_all = df[df['Game_Trigger'] == game_trigger]
    all_countries = sorted(game_data_all['Country'].unique()) if not game_data_all.empty else []
    
    if not all_countries:
        st.warning(f"Ingen data for dette game.")
        return
    ...
    has_blank_group = any(pd.isna(g) or str(g).strip() == '' for g in game_data_all['Group'].unique())
    has_blank_mail = any(pd.isna(m) or str(m).strip() == '' for m in game_data_all['Mail'].unique())
    has_blank_message = any(pd.isna(m) or str(m).strip() == '' for m in game_data_all['Message'].unique())
    has_blank_ab = any(pd.isna(a) or str(a).strip() == '' for a in game_data_all['AB'].unique())
    
    all_groups = sorted([g for g in game_data_all['Group'].unique() if pd.notna(g) and str(g).strip() != ''])
    all_mails_raw = sorted([m for m in game_data_all['Mail'].unique() if pd.notna(m) and str(m).strip() != ''])
    all_messages = sorted([m for m in game_data_all['Message'].unique() if pd.notna(m) and str(m).strip() != ''])
    all_ab = sorted([a for a in game_data_all['AB'].unique() if pd.notna(a) and str(a).strip() != ''])
    ...
    if today.month == 1:
        ...
        prev_month = f"{today.year}-{today.month - 1}"
    recent_months = [current_month, prev_month]
    recent_data = game_data_all[
        (game_data_all['Year_Month'].isin(recent_months)) & 
        (game_data_all['Received_Email'] > 0)
    ]
    ...
    # === SESSION STATE KEYS ===
    ignore_inactive_key = f'gm_ignore_inactive_{game_trigger}'
    group_state_key = f'gm_selected_groups_{game_trigger}'
    group_reset_key = f'gm_cb_reset_group_{game_trigger}'
    mail_state_key = f'gm_selected_mails_{game_trigger}'
    mail_reset_key = f'gm_cb_reset_mail_{game_trigger}'
    message_state_key = f'gm_selected_messages_{game_trigger}'
    message_reset_key = f'gm_cb_reset_message_{game_trigger}'
    ...
    if st.session_state.gm_selected_countries is None:
        ...
        st.session_state.gm_selected_countries = [c for c in st.session_state.gm_selected_countries if c in all_countries]
        if not st.session_state.gm_selected_countries:
            st.session_state.gm_selected_countries = list(all_countries)
    ...
    # === UNDEROVERSKRIFT ===
    st.markdown(f'<p style="color: #9B7EBD; font-size: 1.1em; font-weight: 500; margin-bottom: 0.5em;">{game_trigger}</p>', unsafe_allow_html=True)
    ...
    with col_land:
        ...
        with st.popover(land_label, use_container_width=True):
            reset_land = st.session_state.gm_cb_reset_land
            all_land_selected = len(st.session_state.gm_selected_countries) == len(all_countries)
            select_all_land = st.checkbox("Vælg alle", value=all_land_selected, key=f"gm_sel_all_land_sf_{game_trigger}_{reset_land}")
            ...
            for country in all_countries:
                ...
                with cb_col:
                    checked = country in st.session_state.gm_selected_countries
                    if st.checkbox(country, value=checked, key=f"gm_cb_land_sf_{game_trigger}_{country}_{reset_land}"):
                        new_selected.append(country)
                with only_col:
                    if st.button("Kun", key=f"gm_only_land_{game_trigger}_{country}_{reset_land}", type="secondary"):
                        only_clicked_land = country
    ...
    with col_slider:
        if len(sorted_months) > 1:
            ...
            saved_key = f"gm_month_range_{game_trigger}_saved"
            if saved_key not in st.session_state:
                st.session_state[saved_key] = (default_start, default_end)
            ...
            month_range = st.select_slider(
                ...
                value=initial_value,
                format_func=format_month_short,
                key=f"gm_month_range_{game_trigger}",
                label_visibility="collapsed"
            )
            st.session_state[saved_key] = month_range
            sel_months = get_months_in_range(month_range[0], month_range[1], sorted_months)
    ...
    with col_group:
        ...
        with st.popover(group_label, use_container_width=True):
            ...
            select_all_group = st.checkbox("Vælg alle", value=all_group_selected, key=f"gm_sel_all_group_{game_trigger}_{reset_group}")
            ...
            for group in available_groups:
                ...
                with cb_col:
                    ...
                    if st.checkbox(str(group), value=checked, key=f"gm_cb_group_{game_trigger}_{group}_{reset_group}"):
                        new_selected_groups.append(group)
                with only_col:
                    if st.button("Kun", key=f"gm_only_group_{game_trigger}_{group}_{reset_group}", type="secondary"):
    ...
    with col_mail:
        ...
        with st.popover(mail_label, use_container_width=True):
            ...
            select_all_mail = st.checkbox("Vælg alle", value=all_mail_selected, key=f"gm_sel_all_mail_{game_trigger}_{reset_mail}")
            ...
            for mail in available_mails:
                ...
                with cb_col:
                    ...
                    if st.checkbox(str(mail), value=checked, key=f"gm_cb_mail_{game_trigger}_{mail}_{reset_mail}"):
                        new_selected_mails.append(mail)
                with only_col:
                    if st.button("Kun", key=f"gm_only_mail_{game_trigger}_{mail}_{reset_mail}", type="secondary"):
    ...
    with col_message:
        ...
        with st.popover(message_label, use_container_width=True):
            ...
            select_all_message = st.checkbox("Vælg alle", value=all_message_selected, key=f"gm_sel_all_message_{game_trigger}_{reset_message}")
            ...
            for message in available_messages:
                ...
                with cb_col:
                    ...
                    if st.checkbox(str(message), value=checked, key=f"gm_cb_message_{game_trigger}_{message}_{reset_message}"):
                        new_selected_messages.append(message)
                with only_col:
                    if st.button("Kun", key=f"gm_only_message_{game_trigger}_{message}_{reset_message}", type="secondary"):
    ...
    with col_ab:
        ...
        with st.popover(ab_label, use_container_width=True):
            ...
            select_all_ab = st.checkbox("Vælg alle", value=all_ab_selected, key=f"gm_sel_all_ab_{game_trigger}_{reset_ab}")
            ...
            for ab in available_ab:
                ...
                with cb_col:
                    ...
                    if st.checkbox(str(ab), value=checked, key=f"gm_cb_ab_{game_trigger}_{ab}_{reset_ab}"):
                        new_selected_ab.append(ab)
                with only_col:
                    if st.button("Kun", key=f"gm_only_ab_{game_trigger}_{ab}_{reset_ab}", type="secondary"):
                        only_clicked_ab = ab
    ...
    with col_ig_group:
        ignore_group = st.checkbox(
            ...
            key=f"gm_ignore_group_cb_{game_trigger}"
        )
    ...
    with col_ig_mail:
        ignore_mail = st.checkbox(
            ...
            key=f"gm_ignore_mail_cb_{game_trigger}"
        )
    ...
    with col_ig_message:
        ignore_message = st.checkbox(
            ...
            key=f"gm_ignore_message_cb_{game_trigger}"
        )
    ...
    with col_ig_ab:
        ignore_ab = st.checkbox(
            ...
            key=f"gm_ignore_ab_cb_{game_trigger}"
        )
    ...
    render_single_game_content(df_month_filtered, game_trigger, sel_countries, filter_config, df)

    if st.button('Opdater Data', key=f"gm_refresh_{game_trigger}"):
        st.rerun()
